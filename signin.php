  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12" style="margin-top:5%;position:relative">
        <h3 class="text-center" style="color:aliceblue">FORM TO CONNECT</h3>
        <br>
        <form id="signin" method="POST" action="#">
          <div class="mb-3">
            <label for="a" style="color:aliceblue" class="form-label">UserName</label>
            <input type="text" placeholder="Tên tài khoản" name="ua" class="form-control" required id="a">
          </div>
          <div class="mb-3">
            <label for="b" style="color:aliceblue" class="form-label">Password</label>
            <input type="password" placeholder="Mật khẩu" name="ba" class="form-control" required id="b">
          </div>
        ...
        <form style="margin-bottom:10%;" id="signup" method="POST" action="#">
          ...
          <div class="mb-3">
            ...
            <input type="text" placeholder="Họ và tên" name="fullname" class="form-control" required id="c" aria-describedby="emailHelp">
          </div>
          <div class="mb-3">
        ...
        if (isset($_POST['signinsub'])) {
          ...
          if (mysqli_num_rows($result) != 0) {
            ...
            if ($admin == 1) {
              ...
              echo ("<script>alert('Chúc mừng bạn đã đăng nhậP thành công')</script>");
          ...
          if (mysqli_num_rows($check) == 0) {
            ...
            if ($result) {
              echo ("<script>alert('Chúc mừng bạn đẵ đăng kí thành công')</script>");
            } else {
              echo ("<script>alert('Thất bại')</script>");
            }
          } else {
            echo ("<script>alert('Đã tồn tại tên tài khoản')</script>");
          }
        }
        ?>
        <script>
